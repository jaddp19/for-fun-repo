        ...
        <div class="mb-4">
          <a href="{{ route('admin.role.view') }}" id="no-border"
            class="inline-flex items-center gap-x-2 px-3 py-2 text-sm font-medium
              border border-neutral-700 rounded-lg bg-red-700 text-black   dark:text-white
              hover:bg-red-500 hover:text-white focus:outline-hidden focus:bg-red-500">
            ...
            <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-2">
              @forelse ($this->permissions as $pKey => $permission)
                <label wire:key='{{ $pKey }}' for="hs-checkbox-in-form-{{ $pKey }}" id="no-border"
                  class="flex items-center p-3 w-full bg-red-700 border border-neutral-600 rounded-lg text-sm hover:bg-red-500 hover:text-white transition">
                  <input type="checkbox"
                    id="hs-checkbox-in-form-{{ $pKey }}" wire:model='selectedPermissions'
                    value="{{ $permission->name }}"
                    class="shrink-0 size-4 bg-transparent border-neutral-500 rounded-sm text-black dark:text-white focus:ring-green-600">
